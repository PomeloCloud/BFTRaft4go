...
func (s *BFTRaftServer) GetHost(txn *badger.Txn, nodeId uint64) *pb.Host {
	...
	if item, err := txn.Get(append(ComposeKeyPrefix(HOST_LIST_GROUP, HOST_LIST), U64Bytes(nodeId)...)); err == nil {
		data := ItemValue(item)
		if data == nil {
			return nil
		}
...
func (s *BFTRaftServer) SaveHost(txn *badger.Txn, node *pb.Host) error {
	if data, err := proto.Marshal(node); err == nil {
		dbKey := append(ComposeKeyPrefix(HOST_LIST_GROUP, HOST_LIST), U64Bytes(node.Id)...)
